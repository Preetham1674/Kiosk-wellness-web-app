// frontend/src/pages/Step2_InitialHR.jsx
import React, { useState } from "react";

// Duration of the simulated scan (4 seconds)
const ANIMATION_DURATION_MS = 4000;

const Step2_InitialHR = ({ handleDataInput, nextStep }) => {
  const [isScanning, setIsScanning] = useState(false);
  const [scanComplete, setScanComplete] = useState(false);

  // This function ONLY runs when the user actually clicks the fingerprint
  const startScan = () => {
    if (isScanning || scanComplete) return; // Prevent multiple clicks/scans

    setIsScanning(true);

    // 1. Record the simulated heart rate data internally
    // We use 'false' as the 3rd argument to prevent auto-advancing to Step 3 immediately
    handleDataInput("hrBefore", Math.floor(60 + Math.random() * 30), false);

    // 2. Wait for the 4-second animation duration
    setTimeout(() => {
      setIsScanning(false);
    ...
    <div className="kiosk-card">
      <h2>
        {scanComplete
          ? "Scan Complete!"
          : isScanning
          ? "Recording your rhythm..."
          : "When you're ready, gently place your finger here"}
      </h2>
      <br />

      {/* INTERACTIVE SCANNER:
        The 'active' class is now dynamic. 
        It only applies once the user clicks and starts the scan.
      */}
      <div
        className={`container ${isScanning || scanComplete ? "active" : ""}`}
        onClick={startScan}
        style={{
      ...
      >
        ...
        {/* Success Icon Layer */}
        <svg className="ok" width="100" height="100" viewBox="0 0 100 100">
          <path
      ...
      <div className="animated-button-wrapper">
        <button
          ...
          className={!scanComplete ? "waiting-button" : ""}
        >
          Continue
        </button>
      </div>
...
export default Step2_InitialHR;
