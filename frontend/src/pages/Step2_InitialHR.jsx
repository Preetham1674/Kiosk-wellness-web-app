...
const Step2_InitialHR = ({ handleDataInput, nextStep }) => {
  ...
  const startScan = () => {
    ...
    const generatedBpm = Math.floor(60 + Math.random() * 30);
    setBpm(generatedBpm);
    setIsScanning(true);

    handleDataInput("hrBefore", generatedBpm, false);
  ...
  return (
    <div className="kiosk-card standby-card">
      ...
      >
        {isScanning && <div className="scan-loader"></div>}
      ...
      <div className="bottom-right-anchor">
        ...
        />
        <p className="tap-hint" style={{ opacity: scanComplete ? 0.6 : 0 }}>
          Next
        </p>
      </div>
    </div>
