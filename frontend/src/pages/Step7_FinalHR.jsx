...
const Step7_FinalHR = ({ handleDataInput, submitSession }) => {
  ...
  const startScan = () => {
    if (isScanning || scanComplete) return; // Prevent multiple clicks

    setIsScanning(true);

    // 1. Record the data internally
    handleDataInput("hrAfter", Math.floor(55 + Math.random() * 20), false);

    // 2. Wait for the 4-second animation before showing "Complete" and enabling the button
    setTimeout(() => {
      setIsScanning(false);
    ...
    <div className="kiosk-card">
      <h2>
        {scanComplete
          ...
          : "Gently place your finger on the scanner"}
      </h2>
      <br />

      {/* The 'active' class is now DYNAMIC. 
          It only applies when isScanning is true OR scanComplete is true.
      */}
      <div
      ...
      >
        ...
        {/* Layer 3: OK Checkmark (visible when scanComplete is true) */}
        <svg
          className="ok"
      ...
      <div className="animated-button-wrapper">
        <button
          onClick={submitSession} // This saves the data and navigates to the final results
